    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-100 via-purple-100 to-indigo-200 flex items-center justify-center">
        <div className="text-center">
          <div className="rounded-full h-32 w-32 border-8 border-pink-300 border-t-purple-600 mx-auto mb-4"></div>
          ...
          {Object.entries(courses).map(([courseId, course], index) => {
            ...
            return (
              <Link
                key={courseId}
                to={`/course/${courseId}`}
                className={`group block ${bgClass} rounded-3xl p-6 shadow-xl hover:shadow-2xl transition-all duration-300 border-4 border-white`}
              >
                <div className="text-center">
                  <div className="mb-4 transition-all duration-300">
                    <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto text-2xl font-bold text-white shadow-md"
                      style={{ background: `linear-gradient(135deg, rgba(255,255,255,0.06), rgba(0,0,0,0.06)), ${index % 2 === 0 ? 'linear-gradient(135deg,#ff7a7a,#ffb199)' : 'linear-gradient(135deg,#7aa7ff,#a6d4ff)'}` }}>
                      {course.title.split(' ').map(w => w[0]).slice(0,2).join('')}
                    </div>
                  </div>
                  ...
                  <div className={`inline-flex items-center space-x-2 px-6 py-3 rounded-full bg-gradient-to-r ${colorClass} text-white font-bold text-sm shadow-lg`}>
